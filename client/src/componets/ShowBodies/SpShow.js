...
function Shows (){
    ...
    useEffect(() => {
        const fetchShowsCNT = async () =>{
            ...
            console.log(`Spiritual shows #${count.data[0].total} true false check:${count.data[0].total !== 0}`)
            if(count.data[0].total !== 0 || count.data[0].total !== undefined){
                setPullSwitch(true)
                // console.log(`Spiritual confirm check`)
            }
            else{
                // console.log(`Spiritual fail check`)
                return
            }
        }
        fetchShowsCNT() 
    ...
    if(pullSwith){
        const fetchShows = async () =>{
            const result = await API.getShowCateg(token,{
                ...
                // console.log(`Spiritual show data ${result.data}`)
