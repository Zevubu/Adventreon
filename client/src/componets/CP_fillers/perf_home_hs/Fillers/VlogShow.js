...
function Shows (){
    ...
    useEffect(() => {
        const fetchShowsCNT = async () =>{
            ...
            // console.log(`Performance blog count #${count.data[0].total} true false check:${count.data[0].total !== 0}`)
            if(count.data[0].total !== 0){
                setPullSwitch(true)
                // console.log(`Performance blog confirm check`)
            }
            else{
                // console.log(`Performance blog fail check`)
                return
            }
        }
        fetchShowsCNT() 
    ...
    if(pullSwith){
        const fetchShows = async () =>{
            const result = await API.getShowSubcat(token,{
                ...
                // console.log(`Performance blog show data ${result.data}`)
