...
import {BigBlock,LookBtn,SlideShadowBox,LookBox, H2, HeaderItem,SpHeaderA, DivWBorder} from "../../../../styles/homeStyle";
...
function Shows (){
    ...
    useEffect(() => {
        const fetchShowsCNT = async () =>{
            ...
            // console.log(`Performance story count #${count.data[0].total} true false check:${count.data[0].total !== 0}`)
            if(count.data[0].total !== 0){
                setPullSwitch(true)
                // console.log(`Performance story confirm check`)
            }
            else{
                // console.log(`Performance story fail check`)
                return
            }
        }
        fetchShowsCNT() 
    ...
    if(pullSwith){
        const fetchShows = async () =>{
            const result = await API.getShowSubcat(token,{
                ...
                // console.log(`Performance story show data ${result.data}`)
