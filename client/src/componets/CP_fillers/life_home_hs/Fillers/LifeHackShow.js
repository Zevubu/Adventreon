    ...
    useEffect(() => {
        const fetchShowsCNT = async () =>{
            const count = await API.getShowSubcatNumCheck(token,{
                'category':'life',
                'sub_category':'lifehack'
            })
            // console.log(`life Hack count #${count.data[0].total} true false check:${count.data[0].total !== 0}`)
            if(count.data[0].total !== 0){
                setPullSwitch(true)
                // console.log(`Life lifehack confirm check`)
            }
            else{
                // console.log(`Life lifehack fail check`)
                return
            }
        }
        fetchShowsCNT() 
    ...
    if(pullSwith){
        const fetchShows = async () =>{
            const result = await API.getShowSubcat(token,{
                ...
                // console.log(`life lifehack show data ${result.data}`)
