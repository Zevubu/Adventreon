...
function Shows (){
    ...
    useEffect(() => {
        const fetchShowsCNT = async () =>{
            ...
            // console.log(`Life Counseling count #${count.data[0].total} true false check:${count.data[0].total !== 0}`)
            if(count.data[0].total !== 0){
                setPullSwitch(true)
                // console.log(`Life counseling confirm check`)
            }
            else{
                // console.log(`Life counseling  fail check`)
                return
            }
        }
        fetchShowsCNT() 
    ...
    if(pullSwith){
        const fetchShows = async () =>{
            const result = await API.getShowSubcat(token,{
                ...
                // console.log(`life couseling show data ${result.data}`)
