...
function Shows (){
    ...
    useEffect(() => {
        const fetchShowsCNT = async () =>{
            ...
            // console.log(`Life Cooking count #${count.data[0].total} true false check:${count.data[0].total !== 0}`)
            if(count.data[0].total !== 0){
                setPullSwitch(true)
                // console.log(`Life cooking confirm check`)
            }
            else{
                // console.log(`Life cooking fail check`)
                return
            }
        }
        fetchShowsCNT() 
    ...
    if(pullSwith){
        const fetchShows = async () =>{
            const result = await API.getShowSubcat(token,{
                ...
                // console.log(`life cooking show data ${result.data}`)
