...
function Shows (){
    ...
    useEffect(() => {
        const fetchShowsCNT = async () =>{
            ...
            // console.log(`Music educate count #${count.data[0].total} true false check:${count.data[0].total !== 0}`)
            if(count.data[0].total !== 0){
                setPullSwitch(true)
            //    console.log(`Music educate confirm check`)
            }
            else{
                // console.log(`Music educate fail check`)
                return
            }
        }
        fetchShowsCNT() 
    ...
    if(pullSwith){
        const fetchShows = async () =>{
            const result = await API.getShowSubcat(token,{
                ...
                // console.log(`Music educate show data ${result.data}`)
