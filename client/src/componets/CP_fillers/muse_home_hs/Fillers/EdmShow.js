...
function Shows (){
    ...
    useEffect(() => {
        const fetchShowsCNT = async () =>{
            ...
            // console.log(`Music edm count #${count.data[0].total} true false check:${count.data[0].total !== 0}`)
            if(count.data[0].total !== 0){
                setPullSwitch(true)
                // console.log(`Music edm confirm check`)
            }
            else{
                // console.log(`Music edm  fail check`)
                return
            }
        }
        fetchShowsCNT()  
    ...
    if(pullSwith){
        const fetchShows = async () =>{
            const result = await API.getShowSubcat(token,{
                ...
                // console.log(`Music edm show data ${result.data}`)
