...
function Shows (){
    ...
    useEffect(() => {
        const fetchShowsCNT = async () =>{
            ...
            // console.log(`Music acoustic count #${count.data[0].total} true false check:${count.data[0].total !== 0}`)
            if(count.data[0].total !== 0){
                setPullSwitch(true)
                // console.log(`Music acoustic confirm check`)
            }
            else{
                // console.log(`Music acoustic  fail check`)
                return
            }
        }
        fetchShowsCNT()  
