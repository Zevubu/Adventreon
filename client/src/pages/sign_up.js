    ...
    const UserChecker = (User) =>{
        ...
        if(User.match(stringNameCheck)){
            ...
            API.getUserNameCheck({
                ...
                console.log(`Name used data: ${JSON.stringify(res)}`)
                if(res.data.valid === true){
                    // console.log(`Name used check: ${JSON.stringify(res.data.checker[0].total)}`)
                    if(res.data.checker[0].total !== 0){
                        setUserNameError(true)
                        console.log(`Name used check: ${JSON.stringify(res.data.checker[0].total)}`)
                        console.log(`Name check is total > 0 error:${res.data.err}`)
                    }else if (res.data.checker[0].total === 0){
                        console.log(`Name free check:${res.data.checker[0].total}`)
                        console.log(`Name check total > 0 error:${res.data.err}`)
                        setUserNameError(false)
                    }
                    else{
                        setUserNameError(true)
                        console.log(`Name fail check: ${res.data.checker[0].total}`)
                        console.log(`Name check is valid not total error:${res.data.err}`)
                    }
                }
                else{
                    console.log(`Name check else error:${JSON.stringify(res.data.err)}`)
                    setDbError(true)
                }
            })
        }else{
    ...
    const EmailChecker = (Email) =>{
        ...
        if(Email.match(stringCheck)){
            ...
            API.getEmailCheck({
                ...
                console.log(`Email used data: ${JSON.stringify(res)}`)
                if(res.data.valid === true){
                    if(res.data.checker[0].total !== 0){
                        setEmailError(true)
                        console.log(`Email used check: ${JSON.stringify(res.data.checker[0].total)}`)
                    }else if (res.data.checker[0].total === 0){
                    ...
                    console.log(`Email check error:${res.error}`)
                    setDbError(true)
                }
        ...
        <DivWBorder> 
            ...
            <MarronHeader>
                <H2>Start making positive changes today.</H2>
                 ...
                 {dbError && (
                    <FormBigBox>
                        <H1 color="red">Data base error</H1>
                        <H2 color="black"> Oops something went wrong please try refreshing.</H2>
                        {/* <H2>{dbError}</H2> */}
                    </FormBigBox>
                )}
            {!dbError && (
