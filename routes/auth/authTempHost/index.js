...
// Make new host profile. 

// /auth/temph/register
router.post('/register', async (req, res) => {
    if (!req.body) return res.sendStatus(400);    
    const {first_name, last_name, user_name, dob, email, password, title, about, p_img, b_img, category,sub_catagory, payment, patreon, wp_title, webpage, rsvp_attend, rsvp_perform, verified} = req.body;
        ...
        const result = await create(
            ...
            [first_name, last_name, user_name, 'host', false, dob, email, cryptPass, title, about, p_img, b_img, category,sub_catagory, payment, patreon, wp_title, webpage, rsvp_attend, rsvp_perform, verified]  
        );
        const user = result;
