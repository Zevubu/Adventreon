...
SELECT * FROM users;
SELECT id, first_name, last_name, user_name, user_type, mhswitch, dob, title, about, p_img, b_img, verified FROM users WHERE user_type='host' OR user_type='manager' AND mhswitch=1;
SELECT * FROM users WHERE user_type='host' AND entertain=1 OR mhswitch=1 AND user_type='mod' AND entertain=1;
SELECT * FROM shows;
SELECT * FROM shows WHERE host_id !=2;
SELECT * FROM shows WHERE category='life' ORDER BY RAND() LIMIT 12;
SELECT * FROM shows WHERE category='life' ORDER BY time_stamp DESC LIMIT 12;
